...
public partial class CustomToolWindow : EditorWindow
{
    ...
    // GameEntityAnimationTester.cs 안, isEventRegistered 아래 정도에 추가
    private void UpdateGameEntityAnimationTesterActivation()
    {
        // Animation 탭 & 서브탭 인덱스 1 = "GameEntityAnimationTester"
        bool shouldBeActive =
            selectedMain == MainCategory.Animation &&
            selectedSub == 1;

        if (shouldBeActive)
        {
            // 필요할 때만 등록 (중복 방지는 OnEnable_GameEntityAnimationTester 안에서 처리)
    ...
    {
        RegisterAnimationTesterEvents();
    }

    private void OnLostFocus()
    {
        UnregisterAnimationTesterEvents();
    }
    ...
    // 이벤트 등록
    private void RegisterAnimationTesterEvents()
    {
        if (!isEventRegistered)
        {
            Selection.selectionChanged += RefreshActiveEntity;
    ...
    // 이벤트 해제
    private void UnregisterAnimationTesterEvents()
    {
        if (isEventRegistered)
        {
            Selection.selectionChanged -= RefreshActiveEntity;
