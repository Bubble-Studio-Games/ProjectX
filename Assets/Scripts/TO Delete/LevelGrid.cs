...
public class LevelGrid : MonoBehaviour//, IGridMutation, IGridQuery, IUnitGridManager
{
    ...
    public class GridCellInfo
    {
        ...
        public E_GridCheckType2 gridType; // 필요하다면 체크 타입도 저장

        public GridCellInfo(GameEntity entity, E_GridCheckType2 checkType)
        {
            Entity = entity;
            gridType = checkType;
        }
        ...
        for (int floor = 0; floor < floorAmount; floor++)
        {
            ...
            for (int x = 0; x < width; x++)
                for (int z = 0; z < height; z++)
                    dict[new GridPosition(x, z, floor)] = new GridCellInfo(null, E_GridCheckType2.Void);
    ...
    public void SetCellType(GridPosition gridPosition, E_GridCheckType2 type, GameEntity entity = null)
    {
    ...
    public void SetCellType(IEnumerable<GridPosition> positions, E_GridCheckType2 type, GameEntity entity = null)
    {
    ...
    public GameEntity GetUnitAt(GridPosition gridPosition)
    {
        if (!IsValidGridPosition(gridPosition))
            return null;
    ...
    public E_GridCheckType2 GetCellType(GridPosition gridPosition)
    {
        if (!IsValidGridPosition(gridPosition)) return E_GridCheckType2.Void;
    ...
    public List<(GridPosition, E_GridCheckType2)> GetFloorGridPositionAndType(int floor)
    {
        // 1. floor가 유효한지 확인
        if (!m_DicFloorGridCache.ContainsKey(floor))
            return new List<(GridPosition, E_GridCheckType2)>();

        // 2. 해당 층의 Dictionary<GridPosition, GridCellInfo> 꺼냄
        var floorData = m_DicFloorGridCache[floor];

        // 3. (GridPosition, E_GridCheckType2) 튜플 리스트로 변환해서 반환
        return floorData
            .Select(pair => (pair.Key, pair.Value.gridType)) // ← 여기서 GridType이 enum E_GridCheckType2 타입이라고 가정
            .ToList();
    }
    ...
    /// </summary>
    public List<GridPosition> GetFloorAndTypeGridPositions(int floor, E_GridCheckType2 type)
    {
    ...
    public bool IsGridPositionCheckType(GridPosition gridPosition, IEnumerable<E_GridCheckType2> types)
    {
        return types.Any(type => IsGridPositionCheckType(gridPosition, type));
    }

    public bool IsGridPositionCheckType(GridPosition gridPosition, params E_GridCheckType2[] types)
    {
        ...
        {
            Debug.LogWarning($"⚠️ IsGridCheckType 호출 오류: 최소 1개 이상의 E_GridCheckType2을 전달해야 합니다. (pos: {gridPosition})");
            return false;
        }
    ...
    public bool IsGridPositionCheckType(ICollection<GridPosition> gridPositions, params E_GridCheckType2[] types)
    {
    ...
    public void AddUnitAtGridPositions(IReadOnlyList<GridPosition> cells, GameEntity unit)
    {
        ...
        foreach (var pos in cells)
        {
            ...
            SetCellType(pos, E_GridCheckType2.GameEntity, gridObj.GetTopUnitOrNull());
        }
    }
    ...
    public void RemoveUnitAtGridPositions(IReadOnlyList<GridPosition> cells, GameEntity unit)
    {
        ...
        foreach (var pos in cells)
        {
            ...
            if (gridObj.HasAnyUnit())
                ...
                SetCellType(pos, E_GridCheckType2.Walkable, null);
        }
    }
