...
public class InputBindings : MonoBehaviour, IInputActionMapController
{
    private IInputEvents _events;
    private Define.IMouseClickHandler _mouse; // MouseWorld
    private IRawInputActions _actions;
    private IBuildPlacementService _buildPlacementService;
    ...
    private void Start()
    {
        _mouse = Managers.SceneServices.MouseClickHandler;

        _events = Managers.SceneServices.Get<IInputEvents>();
        _actions = Managers.SceneServices.Get<IRawInputActions>();
        _buildPlacementService = Managers.SceneServices.BuildPlacementService;
    }

    private void OnEnable()
    {
        if(_events == null)
            _events = Managers.SceneServices.InputEvents;
    ...
    private void OnDisable()
    {
        ...
        _events.Unsubscribe(E_InputEvent.MouseLeftDown, OnMouseLeftDown);
        _events.Unsubscribe(E_InputEvent.MouseLeftUp, OnMouseLeftUp);
    }
    ...
    private void OnR()
    {
        if(_buildPlacementService.IsSetuping)
        {
            _buildPlacementService.RotateSelectObject();
        }
        else
        {
    ...
    private void OnMouseLeftDown()
    {
        ...
        _delayLeftDown = StartCoroutine(DelayOneFrame(() => _mouse.MouseDown(E_MouseClickType.Left)));
    }

    private void OnMouseLeftUp()
    {
        if (_delayLeftUp != null) StopCoroutine(_delayLeftUp);
        _delayLeftUp = StartCoroutine(DelayOneFrame(() => _mouse.MouseUp(E_MouseClickType.Left)));
    }
