...
public class SwampBubbleVFX : MonoBehaviour
{
    ...
    private const string AREA_CENTER_STR = "SpawnBox_center";
    ...
    public void Setup(IGridQuery grid)
    {
        _grid = grid;
        CalculateGridBounds();
        InitMaskTexture();
    ...
    {
        if (_meshRenderer == null || _grid == null || Managers.SceneServices.IsNull(_grid))
            return;
        ...
        // 각 코너를 Grid 좌표로 변환 (GridPosition의 x, z를 Vector2Int로 변환)
        GridPosition bottomLeftPos = _grid.GetGridPosition(min);
        GridPosition topRightPos = _grid.GetGridPosition(max);

        Vector2Int bottomLeft = new Vector2Int(bottomLeftPos.x, bottomLeftPos.z);
        Vector2Int topRight = new Vector2Int(topRightPos.x, topRightPos.z);
    ...
    private void InitMaskTexture()
    {
        ...
        Debug.Log("마스크 텍스처 사이즈: " + width + " / " + height);
